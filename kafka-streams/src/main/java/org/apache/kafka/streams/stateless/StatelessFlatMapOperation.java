 ...
 * @author arthur
 */
public class StatelessFlatMapOperation {
    //0. define variable
    private final static Logger LOG = LoggerFactory.getLogger(FirstStreamsApplication.class);
    private final static String APP_ID = "stateless_flatmap_streams_app_id";
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        kStream.flatMap((k, v) -> Arrays.stream(v.split("\\s+"))
                        .map(e -> KeyValue.pair(e, e.length())).collect(Collectors.toList()), Named.as("flatMap-processor"))
                .print(Printed.<String, Integer>toSysOut().withLabel("flatMap-operation"));
        //3. create topology
